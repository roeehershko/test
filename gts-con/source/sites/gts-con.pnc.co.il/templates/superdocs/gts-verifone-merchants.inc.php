<?php
	...
	if ($user['id'] && $user['authenticated']) {
		...
		unsetVar('gts_admin');
		unsetVar('gts_verifone_merchants_without_delete');
		if ($user_object->memberzones && in_array('165310', $user_object->memberzones)) {
			...
			setVar('gts_verifone_merchants_without_delete', true);
	...
	if (!getVar('gts_credentials')) {
		...
		*/
		...
		$active_merchants = $response['data']['active'];
		...
		<div class="tickets">
			<div class="actions">
				<button class="add" style="float:right" onclick="toggle_vf_merchant_form()">+ <?=$strings['add_merchant']?></button>
				...
				<button class="export" onclick="window.open('<?=href($doc.'?action=gts_export_vf_merchants&query='.$_GET['query'].'&date_filter_type='.$_GET['date_filter_type'].'&from_created_date='.$_GET['from_created_date'].'&to_created_date='.$_GET['to_created_date'].'&from_stats_date='.$_GET['from_stats_date'].'&to_stats_date='.$_GET['to_stats_date'])?>')"><?=$strings['export_to_csv']?></button>
            </div>
			<div class="search_container">
	    		...
	    		<form name="search_form" action="" method="GET">
					...
					*/ ?>
					...
					<input type="text" id="from_stats_date__visual" class="date" placeholder="<?=$strings['from_stats_date']?>" value="<?=$_GET['from_stats_date'] ? date('d/m/Y', strtotime($_GET['from_stats_date'])) : false ?>" onkeyup="!$(this).val() ? $('#from_stats_date').val('') : false ">
					<input type="hidden" id="to_stats_date" name="to_stats_date" value="<?=$_GET['to_stats_date']?>">
					<input type="text" id="to_stats_date__visual" class="date" placeholder="<?=$strings['to_stats_date']?>" value="<?=$_GET['to_stats_date'] ? date('d/m/Y', strtotime($_GET['to_stats_date'])) : false ?>" onkeyup="!$(this).val() ? $('#to_stats_date').val('') : false ">
			...
			if (!empty($merchants)) {
				...
				<div class="navigation filters">
					<div class="text"><?=$strings['showing_results']?>: <b><?=$navigation['start']+1?> - <?=$navigation['end']+1?></b> <?=$strings['from']?> <?=$navigation['total']?> | <?=$strings['active_merchants']?> <?=$active_merchants ?: '-' ?></div>
	        	...
	        	for ($i = 0; $i < $limit && $i != count($merchants); $i++) {
	        		...
	        		?>
	        		<div id="vf_merchant_<?=$merchants[$i]['id']?>" class="ticket row merchant" onclick="toggle_vf_merchant_form(this)">
	        			<input type="hidden" class="processor" value="<?=htmlspecialchars($merchants[$i]['processor'])?>">
						...
						<input type="hidden" class="merchant_parent_number" value="<?=$merchants[$i]['parent_number']?>">
	        			...
	        			<div class="ticket_d enable_invoices"><?=$merchants[$i]['invoices']['template'] ? '+' : '-' ?></div>
